...
foreach ($ciudades_actuales as $key => $value) {
  ...
  $array_ciudades [] = $value['ciudad_id'];
}
...
<h2>Armar Centro de Costo <?= $model->nombre?></h2>
<div class="preaviso-form check-form">
 
<?php $form = ActiveForm::begin(['class' => 'check-form']); ?>
  
  <div class="form-group col-md-offset-11 col-sm-offset-11 col-xs-offset-8">
  
      <?= Html::submitButton('Asignar', ['class' =>  'btn btn-primary','name' => 'asignar']) ?>
     ...
     <table class="table" >
        ...
        <tbody>
          ...
          <?php foreach ($ciudades as $key => $value):?>
               <tr>
                  <td><?php if(in_array($value['id'], $array_ciudades)):?>
                      
                      <?= Html::checkBox('ciudad[]',true, ['value' => $value['id']])?>

                      <?php else: ?>

                        <?= Html::checkBox('ciudad[]',false, ['value' => $value['id']])?>
