...
<!-- Validar permiso de administración de armas para poder moodificar datos básicos -->
  <?php if(in_array("adm-armas", $permisos)):?>
    <div class="bottom">
     ...
     <li role="presentation" class="active"><?php echo Html::a('Tipos',Yii::$app->request->baseUrl.'/tipo-arma/index'); ?></li>
...
<?= Html::a('<i class="fa fa-plus"></i>',Yii::$app->request->baseUrl.'/tipo-arma/create',['class'=>'btn btn-primary']) ?>
    
</div>
<?php $form = ActiveForm::begin(['class' => 'check-form']); ?>

<h3 style="text-align: center;">Tipos de Armas</h3>
<table class="table">
  <thead>    
    <tr>       
       <th>Nombre</th>
...
?>
    <tr>
       ...
       <td><?php
                echo Html::a('<i class="fa fa-pencil fa-fw"></i>',Yii::$app->request->baseUrl.'/tipo-arma/update?id='.$value->id);
                echo Html::a('<i class="fa fa-remove"></i>',Yii::$app->request->baseUrl.'/tipo-arma/delete?id='.$value->id);
