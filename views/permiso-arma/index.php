...
<!-- Validar permiso de administración de armas para poder moodificar datos básicos -->
  <?php if(in_array("adm-armas", $permisos)):?>
    <div class="bottom">
     ...
     <li role="presentation" class="active"><?php echo Html::a('Permiso Arma',Yii::$app->request->baseUrl.'/permiso-arma/index'); ?></li>
     <li role="presentation"><?php echo Html::a('Armas',Yii::$app->request->baseUrl.'/arma/index'); ?></li>
...
<?= Html::a('<i class="fa fa-plus"></i>',Yii::$app->request->baseUrl.'/permiso-arma/create',['class'=>'btn btn-primary']) ?>
    
</div>
<?php $form = ActiveForm::begin(['class' => 'check-form']); ?>

<h3 style="text-align: center;">Permisos de Armas</h3>
<table class="table">
  <thead>    
    <tr>       
       <th>Nombre</th>
...
?>
    <tr>
       ...
       <td><?php
                echo Html::a('<i class="fa fa-pencil fa-fw"></i>',Yii::$app->request->baseUrl.'/permiso-arma/update?id='.$value->id);
                echo Html::a('<i class="fa fa-remove"></i>',Yii::$app->request->baseUrl.'/permiso-arma/delete?id='.$value->id);
