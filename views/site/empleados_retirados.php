...
<div class="preaviso-form check-form">
 ...
 <?= $this->render('_tabs',['retirados'=>'active']) ?>

   <h2>Empleados Retirados</h2>
<?php $form = ActiveForm::begin(['class' => 'check-form']); ?>
<div class="table-responsive"> 
<table class="table display my-data3" data-page-length='50' cellspacing="0" width="100%">
...
?>
    <tr>
       <td><?php echo Html::a($value['NIT'],Yii::$app->request->baseUrl.'/site/detalleemp-retirados?id='.$value['NIT']);?></td>
       
       <td><?=$value['NOMBRE']?></td>
       <td><?=$value['UBICACION']?></td>
       <td>
       	<?php 
       		$AuditoriaRetirados=AuditoriaRetirados::find()->where('id_tercero='.$value['NIT'])->one();
       		if($AuditoriaRetirados!=null){
       			switch ($AuditoriaRetirados->estado) {
       				case 'A':
       					echo "<label class='label label-success'>Apto</label>";
       					break;
       				
       				case 'N':
       					echo "<label class='label label-danger'>No Apto</label>";
       					break;
       			}
       ...
       <td><?= date("d/m/Y ",strtotime($value['FECHA_RETIRO']))?></td>
