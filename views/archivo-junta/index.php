...
<?= Html::a('<i class="fa fa-arrow-left"></i>',Yii::$app->request->baseUrl.'/junta/index',['class'=>'btn btn-primary']) ?>
<?php if(in_array("archivo-junta", $permisos)):?>   
<?= Html::a('<i class="fa fa-plus"></i>',Yii::$app->request->baseUrl.'/archivo-junta/create?id_junta='.$id_junta,['class'=>'btn btn-primary pull-right']) ?>
<?php endif;?>       
</div>
    ...
    <h3 style="text-align: center;">
        <?php 
              if(count($archivos) > 0){
                  ...
                  echo 'Archivos junta '.$archivos[0]->junta->titulo;
              }else{

                  echo 'Archivos de junta';
              }
        ?>
    </h3>    
</div>
...
?>
    <tr>
      <td><?=Html::a('<i class="fa fa-remove fa-fw"></i>',Yii::$app->request->baseUrl.'/archivo-junta/empanada?id='.$value->id)?></td></td> 
      <td><?=Html::a('<i class="fa fa-eye fa-fw"></i>',Yii::$app->request->baseUrl.'/archivo-junta/viewpdf?id=http://181.49.3.226:1580'.Yii::$app->request->baseUrl.$value->archivo.'&id_junta='.$id_junta,['title' => 'Descargar'])?></td>       
